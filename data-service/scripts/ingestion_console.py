...
class App:
    ...
    def __init__(self,root,state,target):
        ...
        self.pipeline_status_var=tk.StringVar(value='آماده'); self.local_status_var=tk.StringVar(); self.server_status_var=tk.StringVar(value='بررسی نشده'); self.last_run_var=tk.StringVar(); self.manager_status_var=tk.StringVar(value='بدون عملیات فعال'); self.busy=False
    ...
    def build_operations_tab(self, tab):
        ...
        for title,var in (('وضعیت لوکال',self.local_status_var),('وضعیت سرور',self.server_status_var),('عملیات خودکار',self.manager_status_var),('آخرین اجرا',self.last_run_var)):
    ...
    def refresh_operations(self):
        ...
        try:
            from scripts.database_operation_manager import summary as operation_summary
            operation=operation_summary();eta=operation.get('eta_seconds');eta_text='نامشخص' if eta is None else f'حدود {max(1,round(eta/60))} دقیقه'
            self.manager_status_var.set(fa(f'{"در حال اجرا" if operation["active"] else operation["status"]}\nپیشرفت: {operation["done_symbols"]}/{operation["total_symbols"]} | باقیمانده: {operation["remaining_symbols"]}\nزمان تقریبی: {eta_text}'))
        except Exception as exc:
            self.manager_status_var.set(fa('وضعیت مدیر عملیات در دسترس نیست: '+str(exc)))
