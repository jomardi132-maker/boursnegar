...
class App:
    ...
    def __init__(self,root,state,target):
        ...
        notebook=ttk.Notebook(root); notebook.pack(fill='both',expand=True,padx=8,pady=8); symbols_tab=ttk.Frame(notebook); files_tab=ttk.Frame(notebook); notebook.add(symbols_tab,text=fa('وضعیت نمادها')); notebook.add(files_tab,text=fa('دفترکل فایل‌های محلی'))
    ...
    def run_auto(self, send):
        ...
        def work():
            ...
            try: command(cmd,self.log); self.events.put(('refresh',None)); self.log('فرایند خودکار با موفقیت پایان یافت')
            except Exception as exc: self.log('خطای فرایند خودکار: '+str(exc))
        threading.Thread(target=work,daemon=True).start()
    def drain(self):
