...
SELECT a.symbol, coalesce(ind.model_family,'unclassified') model_family,
  ...
  coalesce(f.ocf,0)>0 ocf_gate, coalesce(f.equity,0)>0 equity_gate,
...
def main() -> None:
    ...
    gates = ('shares_gate','ocf_gate','equity_gate','sustainable_profit_gate','unit_gate')
    for row in rows:
        passed = sum(bool(row[g]) for g in gates)
        row['status'] = 'PASS' if passed == len(gates) else 'REVIEW'
        row['passed_gates'] = passed
    result = {'method':'valuation_input_gate_audit','gates':list(gates),
