...
def main():
    ...
    for directory in (browser,norm,codal,events): directory.mkdir(parents=True,exist_ok=True)
    if a.codalpy_first:
    ...
    if a.symbol:
        ...
        cmd += ['--from-jalali',a.from_jalali,'--to-jalali',a.to_jalali,'--out',str(browser),'--port',str(a.chrome_port),'--profile',a.profile]
        for enabled,flag in ((a.download_documents,'--download-documents'),(a.download_all_documents,'--download-all-documents'),(a.professional_documents,'--professional-documents'),(a.excel_only,'--excel-only'),(a.html_only,'--html-only'),(a.defer_pdf,'--defer-pdf')):
            if enabled: cmd.append(flag)
        run(cmd,a.dry_run,180); checkpoint=browser/'checkpoint.json'; browser_had_errors=bool(json.loads(checkpoint.read_text()).get('errors')) if checkpoint.exists() and not a.dry_run else False
        run([PYTHON,'data-service/scripts/normalize_browser_statements.py','--capture',str(browser),'--out',str(norm)],a.dry_run); run([PYTHON,'data-service/scripts/normalize_notice_events.py','--capture',str(browser),'--out',str(events)],a.dry_run); run([PYTHON,'data-service/scripts/build_local_codal_db.py','--db',a.local_db,'--artifact',str(browser),'--artifact',str(norm),'--artifact',str(events)],a.dry_run)
