...
describe('platform routes security contract',()=>{
 ...
 it.each(['/api/admin/payments','/api/admin/users','/api/admin/audit','/api/admin/plans','/api/admin/campaigns','/api/admin/settings','/api/admin/sms'])('protects admin route %s',route=>{const block=source.slice(source.indexOf(route),source.indexOf(route)+220);expect(block).toContain('requireUser');expect(block).toContain('requireAdmin');});
 ...
 it('never exposes secret-shaped system settings',()=>expect(source).toMatch(/secret\|token\|password\|api/));
});
