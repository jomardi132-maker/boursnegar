...
function AuthDialog({ onClose, onLogin }: { onClose: () => void; onLogin: (u: User, csrf: string) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(e)=>e.target===e.currentTarget&&close()}><div className="auth-dialog" role="dialog" aria-modal="true" aria-labelledby="auth-title"><button className="close" onClick={close} aria-label="بستن"><X/></button><span className="brand-mark"><ShieldCheck/></span><h2 id="auth-title">{title}</h2><p>{mode==='forgot'?'ایمیل حساب را وارد کنید تا لینک امن بازیابی ارسال شود.':mode==='reset'?'رمز تازه باید دست‌کم ۸ نویسه باشد.':'با ایمیل و رمز عبور امن وارد شوید.'}</p><form onSubmit={submit}>{mode!=='reset'&&<><label htmlFor="auth-email">ایمیل</label><input id="auth-email" dir="ltr" type="email" autoComplete="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="name@example.com" required/></>} {mode!=='forgot'&&<><label htmlFor="auth-password">رمز عبور</label><input id="auth-password" dir="ltr" type="password" minLength={mode==='login'?1:8} maxLength={128} autoComplete={mode==='login'?'current-password':'new-password'} value={password} onChange={e=>setPassword(e.target.value)} required/></>} {error&&<div className="error-state" role="alert">{error}</div>}{notice&&<div className="success-state" role="status">{notice}</div>}<button className="button primary" disabled={busy}>{busy?'لطفاً صبر کنید':mode==='login'?'ورود':mode==='register'?'ساخت حساب':mode==='forgot'?'ارسال لینک بازیابی':'ثبت رمز جدید'}</button></form><div className="auth-switch">{mode==='login'&&<><button type="button" onClick={()=>setMode('forgot')}>رمز را فراموش کرده‌اید؟</button><button type="button" onClick={()=>setMode('register')}>حساب ندارید؟ ثبت‌نام</button></>}{mode==='register'&&<button type="button" onClick={()=>setMode('login')}>حساب دارید؟ وارد شوید</button>}{mode==='forgot'&&<button type="button" onClick={()=>setMode('login')}>بازگشت به ورود</button>}</div><small>رمز عبور به‌صورت hash نگهداری می‌شود و هرگز با ایمیل ارسال یا قابل بازیابی نیست.</small></div></div>;
}
