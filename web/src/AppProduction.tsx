...
export function AppProduction() {
  ...
  useEffect(() => {
    api<MarketOverview>('/api/market/overview')
      .then((value) => value?.catalog?.instruments != null ? setOverview(value) : setOverview(null))
...
function AuthDialog({ onClose, onLogin }: { onClose: () => void; onLogin: (u: User, csrf: string) => void }) {
  ...
  async function submit(e:FormEvent){e.preventDefault();setBusy(true);setError('');setNotice('');try{
    ...
    const r=await api<{user:User;csrfToken:string;verificationRequired?:boolean}>(endpoint,{method:'POST',body:JSON.stringify({email,password,referralCode})});if(r.verificationRequired){setMode('verify');setNotice('کد تأیید به ایمیل شما ارسال شد.');return;}onLogin(r.user,r.csrfToken);
