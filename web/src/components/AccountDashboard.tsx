...
function DataStatus({ data }: { data: any }) {
  ...
  const [message,setMessage]=useState("");
  async function refresh(pipeline:"market"|"codal") {
    setRefreshing(pipeline); setMessage("");
    try { const result:any=await api(`/api/admin/data-refresh/${pipeline}`,{method:"POST",body:"{}"}); setMessage(result.message); }
    catch(error:any) { setMessage(error?.message||"اجرای به‌روزرسانی ممکن نشد."); }
  ...
  return <><div className="dashboard-cards data-cards">{Object.entries(data.counts || {}).map(([key,value])=><article key={key}><small>{labels[key]||key}</small><strong>{typeof value==='number'?value.toLocaleString('fa-IR'):value?new Date(String(value)).toLocaleString('fa-IR'):'—'}</strong></article>)}</div><section className="dashboard-section"><h3>سلامت منابع و تازه‌سازی</h3><div className="dashboard-list"><article><span><b>موتور تحلیل</b><br/>{data.providers?.data_service?.ok?"در دسترس":"قطع یا کند"}</span><em className={`run-status ${data.providers?.data_service?.ok?"success":"failed"}`}>{data.providers?.data_service?.status||"OFF"}</em></article><article><span><b>قیمت‌های بازار</b><br/>{data.providers?.market?.status||"بدون اجرای ثبت‌شده"}</span><button disabled={!!refreshing} onClick={()=>refresh("market")}>{refreshing==="market"?"در حال ارسال…":"به‌روزرسانی قیمت"}</button></article><article><span><b>اطلاعیه‌های کدال</b><br/>{data.providers?.codal?.status||"بدون اجرای ثبت‌شده"}</span><button disabled={!!refreshing} onClick={()=>refresh("codal")}>{refreshing==="codal"?"در حال ارسال…":"به‌روزرسانی کدال"}</button></article></div>{message&&<p className="success-state">{message}</p>}</section><section className="dashboard-section"><h3>آخرین اجرای خط‌های داده</h3><div className="dashboard-list">{(data.pipelines||[]).map((run:any,i:number)=><article key={`${run.pipeline}-${run.started_at}-${i}`}><span><b>{run.pipeline}</b><br/>{new Date(run.started_at).toLocaleString('fa-IR')} · {run.error_summary||JSON.stringify(run.metrics)}</span><em className={`run-status ${String(run.status).toLowerCase()}`}>{run.status}</em></article>)}</div></section>{(data.issues||[]).length>0&&<section className="dashboard-section"><h3>هشدارهای کیفیت باز</h3><div className="dashboard-list">{data.issues.map((issue:any,i:number)=><article key={`${issue.issue_code}-${i}`}><span><b>{issue.issue_code}</b><br/>{issue.cause}</span><em>{issue.severity}</em></article>)}</div></section>}</>;
}
function AdminUsers({
  initial,
  reload,
