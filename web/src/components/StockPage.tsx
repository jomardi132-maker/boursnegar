...
function SnapshotCard({ snapshot }: { snapshot: Payload["snapshot"] }) {
  ...
  return <article className={`snapshot-card ${snapshot.decision?.toLowerCase()}`}><header><span>آخرین تحلیل ثبت‌شده</span><em>{decisionFa[snapshot.decision] || snapshot.decision}</em></header><div className="score-ring" style={{"--score": `${Number(snapshot.score || 0) * 3.6}deg`} as CSSProperties}><strong>{fa(snapshot.score,0)}</strong><small>امتیاز سلامت</small></div><div className="snapshot-metrics"><span>پوشش داده <b>{fa(snapshot.coverage,0)}٪</b></span><span>اطمینان <b>{fa(snapshot.confidence,0)}٪</b></span></div>{actionable && snapshot.fair_value_base && <div className="fair-band"><small>ارزش منصفانهٔ سناریوی پایه</small><b>{fa(snapshot.fair_value_base)} ریال</b><span>{fa(snapshot.fair_value_low)} تا {fa(snapshot.fair_value_high)}</span></div>}{!actionable && <p className="snapshot-note">برای صدور ارزش‌گذاری و نتیجهٔ قابل‌اقدام، دادهٔ معتبر بیشتری لازم است.</p>}<small className="snapshot-date">محاسبه: {new Date(snapshot.calculated_at).toLocaleString("fa-IR")}</small></article>;
}
