...
type Payload = {
  ...
  latest: Price | null; periodReturn: number | null; prices: Price[];
...
const decisionFa: Record<string, string> = { BUY: "ارزنده", HOLD: "نگهداری / بررسی", SELL: "گران", INSUFFICIENT_DATA: "داده ناکافی" };

export function StockPage({ symbol, onAnalyze }: { symbol: string; onAnalyze: (symbol: string) => void }) {
  ...
  const positive = Number(data.periodReturn || 0) >= 0;
  return <div className="stock-shell" dir="rtl">
    ...
    <main className="stock-page">
      ...
      <section className="stock-grid">
        <article className="chart-card"><header><div><span>روند قیمت پایانی</span><h2>نمودار عملکرد سهم</h2></div><span className={positive ? "trend positive" : "trend negative"}>{positive ? <TrendingUp/> : <TrendingDown/>}{fa(data.periodReturn,1)}٪</span></header><PriceChart prices={data.prices}/><footer><span><i className="legend-price"/> قیمت پایانی (ریال)</span><span>منبع: TSETMC · بدون تعدیل رویداد شرکتی</span></footer></article>
...
function SnapshotCard({ snapshot }: { snapshot: Payload["snapshot"] }) {
  if (!snapshot) return <article className="snapshot-card empty"><Database/><h2>تحلیل هنوز ساخته نشده</h2><p>اولین تحلیل کاربر، نسخهٔ تاریخ‌دار و قابل ردگیری این نماد را ایجاد می‌کند.</p></article>;
...
function PriceChart({ prices }: { prices: Price[] }) {
  const points = useMemo(() => {
    const values = prices.map(p => Number(p.adjusted_close ?? p.close)).filter(Number.isFinite);
