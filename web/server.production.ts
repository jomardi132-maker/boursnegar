...
app.get("/api/comments", asyncRoute(async (req, res) => {
  ...
  const rows = await pool.query(`SELECT c.id,c.parent_id,c.body,c.created_at,coalesce(split_part(e.email,'@',1),'کاربر') AS user_label,(SELECT count(*)::int FROM comment_likes cl WHERE cl.comment_id=c.id) AS likes,EXISTS(SELECT 1 FROM comment_likes cl WHERE cl.comment_id=c.id AND cl.user_id=$3::uuid) AS user_liked FROM comments c JOIN users u ON u.id=c.user_id LEFT JOIN email_identities e ON e.user_id=u.id WHERE c.kind=$1 AND ($2::text IS NULL OR c.symbol=$2) AND c.status='published' ORDER BY c.created_at ASC LIMIT 500`, [kind, symbol, req.authUser?.id ?? null]);
...
app.post("/api/comments/:id/like",requireUser,requireCsrf,rateLimit("comment-like",30,15*60_000),asyncRoute(async(req,res)=>{const found=await pool.query(`SELECT id FROM comments WHERE id=$1 AND status='published'`,[req.params.id]);if(!found.rowCount)return res.status(404).json({success:false,error:"نظر پیدا نشد."});const removed=await pool.query(`DELETE FROM comment_likes WHERE comment_id=$1 AND user_id=$2 RETURNING comment_id`,[req.params.id,req.authUser!.id]);if(!removed.rowCount)await pool.query(`INSERT INTO comment_likes(comment_id,user_id) VALUES($1,$2) ON CONFLICT DO NOTHING`,[req.params.id,req.authUser!.id]);const count=await pool.query(`SELECT count(*)::int AS likes FROM comment_likes WHERE comment_id=$1`,[req.params.id]);res.json({success:true,liked:!removed.rowCount,likes:count.rows[0].likes});}));
app.post("/api/comments/:id/report",requireUser,requireCsrf,rateLimit("comment-report",10,60*60_000),asyncRoute(async(req,res)=>{const parsed=z.object({reason:z.string().trim().min(3).max(240).default("نامناسب")}).safeParse(req.body);if(!parsed.success)return res.status(400).json({success:false,error:"دلیل گزارش معتبر نیست."});await pool.query(`INSERT INTO comment_reports(comment_id,user_id,reason) SELECT id,$2,$3 FROM comments WHERE id=$1 AND status='published' ON CONFLICT(comment_id,user_id) DO UPDATE SET reason=excluded.reason,status='pending'`,[req.params.id,req.authUser!.id,parsed.data.reason]);res.status(202).json({success:true,message:"گزارش برای بررسی ثبت شد."});}));
app.get("/api/account/notifications",requireUser,asyncRoute(async(req,res)=>{const rows=await pool.query(`SELECT id,kind,title,body,target_url,read_at,created_at FROM user_notifications WHERE user_id=$1 ORDER BY created_at DESC LIMIT 100`,[req.authUser!.id]);res.json({success:true,notifications:rows.rows,unread:rows.rows.filter(x=>!x.read_at).length});}));
app.post(
