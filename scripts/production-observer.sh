...
if any(str(row.get("data_coverage")) != "100.0" for row in rows):
    raise SystemExit("screener coverage below 100 percent")
PY
...
printf 'BOURSNEGAR_OBSERVER=PASS disk=%s%% rows=50 coverage=100%%\n' "$used"
