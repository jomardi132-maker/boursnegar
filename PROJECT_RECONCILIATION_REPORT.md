...
- mirror محلی `artifacts/local-ingestion.sqlite3` از artifactهای v16/v17/v18 به‌صورت idempotent بازسازی شد: ۱۵۲۴ نماد، ۱۵۴۸۳ اطلاعیه، ۲۰۵۹۸ fact و ۱۴۴۵۵ رویداد ثبت شده است. پس از بازمحاسبهٔ tierها بر اساس fact و دورهٔ محلی، ۲۹۷ نماد `complete`، ۳۵۹ نماد `comparable` و ۸۶۸ نماد `incomplete` هستند؛ این معیار محلی با وضعیت قدیمی remote-derived جایگزین شد.
...
- دفترکل فایل‌محور روی هر سه مجموعهٔ v16/v17/v18 ساخته شد: ۴۶۶۳ manifest، تعداد ۳۲۷۵۸ فایل موجود و ۱٬۱۹۳٬۱۹۳ رکورد اعلام‌شده بررسی شدند. هیچ فایل اعلام‌شده‌ای مفقود نبود؛ ۱۹۵۸۵ فایل با checksum مورد انتظار یا checksum شناخته‌شدهٔ یک artifact دیگر تطبیق داشت، ۲۶۷۴ فایل به رکوردهای واردشده در SQLite متصل شد، ۷۰۶۴ فایل مدیریتی (`manifest`/`checkpoint`/log/other) فهرست شد و هیچ فایل `QUARANTINED` یا `FAILED` مشاهده نشد. ۳۴۳۵ فایل واقعاً بدون شناسنامهٔ قابل اتصال باقی مانده‌اند: ۱۵۰ JSONL و ۳۲۸۵ سند، که ۳۲۰۱ مورد از سندها Excel هستند و باید بدون انتساب حدسی، با parser و provenance مستقل بررسی شوند.
...
- از ۷۲۶ fact آماده، scope برای ۶۹۹ مورد مستند است: ۱۱۹ مورد `consolidated` و ۵۸۰ مورد `separate`؛ ۲۷ مورد بدون scope باقی مانده و عمداً از ورود canonical کنار گذاشته شده‌اند. پس از کنترل idempotency، ۶۴۹ fact جدید وارد facts محلی شد و ۵۰ مورد تکراری تشخیص داده شد؛ ۶۹۹ کاندیدا اکنون `PROMOTED_LOCAL` هستند.
