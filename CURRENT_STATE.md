...
- timer ساعتی `boursnegar-codal-financials` بازنشسته شد: manifest ثابت از `2026-08-23` ده‌ها بار با `inserted=0` و `standard_facts=0` replay شده بود. timer در Production disable و unit آن پس از backup به `/var/backups/boursnegar/boursnegar-codal-financials.timer-retired-20260907T085230Z` منتقل شد؛ service دستی diagnostic حفظ شد. regression repository مانع بازگشت timer است و suite به `149/149` رسید؛ PR `#58` با CI سبز در main ادغام شد.
- timer پنج‌دقیقه‌ای alert-worker نیز در Production disable شد، زیرا `ALERT_WORKER_ENABLED=false`، `SMS_ENABLED=false` و تعداد alert فعال صفر بود و فقط log تکراری `alert-worker disabled` تولید می‌کرد. unit/service حذف نشدند و فعال‌سازی آینده قابل‌بازگشت است. observer پس از توقف هر دو timer `PASS`, `failed_units=0`, `alert_worker=not-scheduled` گزارش کرد.
- آخرین artifact collector محلی برای `14050616` سه manifest معتبر دارد اما `normalized.jsonl` صفر بایت است؛ بنابراین delta قابل promotion وجود ندارد و هیچ backup/import/report تازه‌ای اجرا نشد.
- observer Production اکنون activeبودن چهار timer ضروری market-daily، market-intraday، snapshot-refresh و backup-retention را enforce می‌کند و بازگشت timer Codal یا اجرای alert timer هنگام خاموش‌بودن feature/SMS را FAIL می‌کند. نسخهٔ جدید پس از backup با `required_timers=4` PASS شد؛ regression suite برابر `150/150` و PR `#61` با CI سبز در main ادغام شد.
- cleanup محدود releaseها پس از اثبات عدم reference و نبود artifact مالی انجام شد: دو web release قدیمی `20260830T022500Z-ratio-quality` و `20260831T-fund-model-ui` که فقط build/node_modules تکراری داشتند، و staging ناموفق و هرگز فعال‌نشدهٔ data release `20260907T0835Z-main-74d2aa3b` حذف شدند. این مسیرها روی سرور قابل restore مستقیم نیستند اما web buildها از Git بازتولیدپذیرند و staging سوم هرگز release معتبر نبود. active/rollback و release حاوی staging شواهد حفظ شدند؛ disk از `71%` به `70%` رسید و observer PASS ماند.
- retention به‌علت ایجاد backup تازهٔ schema-registry ابتدا dry-run و سپس طبق policy دو-dump apply شد. سه dump قدیمی `20260907T072557Z-evidence-before.dump`، `20260907T010507Z-events-before.dump` و `20260907T005421Z-events-before.dump` حذف و مستقیماً قابل‌بازیابی نیستند؛ دو rollback جدیدتر و معتبر با `pg_restore -l` حفظ شدند: `20260907T072842Z-evidence-before.dump` با SHA-256 `15565b9617caba11a2037ce0ddf92a439614c59889c4706449cf490603fc8f70` و `20260907T083226Z-schema-registry-before.dump` با SHA-256 `40c8a8caa3314d90c85ab8d6d8748d5d5800f321dae3571c01943a6f4a1ea94d`. disk به `69%` رسید و observer PASS ماند.

## 2026-09-07 - registry ابزارها و baseline واقعی FCFE

- ۵۴ CLI پایتون data-service در `data-service/scripts/README.md` به مسیر canonical، ممیزی read-only، تغییر دادهٔ gated، بازارِ deferred و legacy تقسیم شدند. contract جدید مانع ورود اسکریپت ثبت‌نشده می‌شود؛ suite کامل `155/155` موفق است.
- `run_all_incomplete.py` و `backfill_codal_1404.py` بدون `--acknowledge-historical-batch` متوقف می‌شوند؛ هیچ batch تاریخی در این تغییر اجرا نشد.
...
## 2026-09-07 - تکمیل یک delta واقعی NAV صندوق‌ها

- ممیزی read-only صندوق‌ها نشان داد از `419` صندوق فعال، `80` صندوق NAV معتبر و `74` صندوق NAV و تعداد واحد معتبرِ هم‌دوره داشتند. شش نماد NAV-only بودند: «دانیک»، «دیبا»، «سپر»، «هم ارز»، «پایدار» و «پتروصبا».
- در artifact موجود `fund-nav-followup-20260906-batch146` برای «دانیک» سه رکورد رسمی `units_outstanding=150` با هویت دوره، source action و SHA-256 معتبر پیدا شد. هر ۸ سند و ۳۹ رکورد normalized این batch بدون خطا با manifest تطبیق داشتند؛ precheck Production فقط همین سه action را pending نشان داد.
- promotion استاندارد پس از backup `/var/backups/boursnegar/20260907T124331Z-evidence-before.dump` انجام شد: اجرای اول `inserted=3`, `standard_facts=3`, `validation_errors=[]` و replay برابر `inserted=0`, `standard_facts=0` بود. backup با `pg_restore -l` و SHA-256 `b09db3ecdd94ad3365f781563effd1ac9ea3f44f653e6dab3d7da17a1b3ab5ae` معتبر است؛ health/readiness سبز ماند.
- ممیزی read-only پس از promotion نتیجهٔ واقعی `419` صندوق فعال، `80` دارای NAV، `75` دارای units و `75` دارای NAV+units هم‌دوره را ثبت کرد. پنج NAV-only باقی‌مانده دقیقاً «دیبا»، «سپر»، «هم ارز»، «پایدار» و «پتروصبا» هستند؛ بدون evidence تازه ممیزی یا promotion آن‌ها تکرار نمی‌شود.
- retention تأییدشدهٔ دو dump ابتدا dry-run و سپس apply شد. dump قدیمی `/var/backups/boursnegar/20260907T072842Z-evidence-before.dump` حذف و مستقیماً قابل‌بازیابی نیست؛ dumpهای معتبر `20260907T083226Z-schema-registry-before.dump` و `20260907T124331Z-evidence-before.dump` حفظ شدند.
- importer اصلاح شد تا default گزارش `*` باشد و در فایل‌های چندنمادی، فیلتر symbol در سطح هر رکورد اعمال شود؛ در نتیجه گزارش promotion دیگر نماد پیش‌فرض گمراه‌کننده ندارد و انتخاب symbol نمی‌تواند رکوردهای دیگر همان فایل را وارد کند. auditor موجود NAV نیز readiness صندوق را با گیت صریح `VALID`، واحد canonical، مقدار مثبت و دورهٔ یکسان گزارش می‌کند. suite کامل data-service برابر `163/163` و `git diff --check` موفق است.
- طبق تصمیم کاربر، بازار، backtest و timerهای روزانه تا پایان سایر کارها خارج از محدوده‌اند؛ این مرحله هیچ تغییری در آن‌ها نداد.
- اصلاحات با `163/163` تست و CI رسمی data-service/web در PR `#71` ادغام شد؛ merge commit برابر `f2b6240d0` است. release اتمی `/var/www/boursnegar-data-releases/20260907T181233Z-main-f2b6240d0` فعال و release `20260907T123202Z-main-80c5f3796` به‌عنوان rollback حفظ شد. backup فایل‌های جایگزین‌شده `/var/backups/boursnegar/20260907T181233Z-fund-tools-before.tar.gz` با SHA-256 `96077b6acbbed8a8e0b08b2823e02c0464ad6d8e12568b00df61437d00a6b050` است. نخستین health-check دو ثانیه پس از restart زودهنگام بود و connection refused داد؛ بررسی بعد از تکمیل startup، `/healthz=ok`، `/readyz=ready`، observer موفق، سرویس `active/running` و `NRestarts=0` را تأیید کرد.

## 2026-09-07 - بستن کامل صف NAV-only صندوق‌ها

- بازپردازش پنج Excel رسمی موجود نشان داد parser فعلی ردیف صریح «خالص دارایی‌ها (واحدهای سرمایه‌گذاری) پایان دوره» را درست به `units_outstanding` می‌خواند، اما normalized artifactهای قدیمی پیش از این قابلیت ساخته شده بودند. این اعداد مستقیماً از سند رسمی آمدند و از NAV یا خالص دارایی استنتاج نشدند.
- مجموعهٔ curated فقط پنج source منطبق با NAVهای معتبر را نگه داشت: «دیبا» `497598421`، «سپر» `11013`، «هم ارز» `176`، «پایدار» `3000000` و «پتروصبا» `1349691086` واحد. پنج سند، `25` fact و manifest/SHA-256 با `errors=0` اعتبارسنجی شدند؛ actionهای units پیش از import در Production وجود نداشتند.
- promotion با backup `/var/backups/boursnegar/20260907T182020Z-evidence-before.dump` انجام شد؛ backup با `pg_restore -l` و SHA-256 `537906f3724b8d61f0873bf30977d3e5d39eb3037e47b2bdd18f9dfb8ccf9644` معتبر است. import دقیقاً `inserted=5`, `standard_facts=5`, `validation_errors=[]` و replay برابر `inserted=0`, `standard_facts=0` بود؛ readiness سبز ماند.
- ممیزی read-only نهایی با artifact محلی SHA-256 `dbc75de46d15f2bb8f7a425124b93f7c7fbe80bb6416e06375c1ab7dfd633b67` نتیجه داد: `419` صندوق فعال، `80` دارای NAV معتبر، `80` دارای units و `80` دارای NAV+units هم‌دوره؛ فهرست NAV-only خالی است. `339` صندوق بدون NAV معتبر همچنان evidence-gated هستند.
- retention دو-dump در dry-run `20260907T182321Z` و apply `20260907T182328Z` اجرا شد. dump قدیمی schema-registry با SHA-256 `40c8a8caa3314d90c85ab8d6d8748d5d5800f321dae3571c01943a6f4a1ea94d` حذف و مستقیماً قابل‌بازیابی نیست؛ dumpهای `20260907T124331Z-evidence-before.dump` و `20260907T182020Z-evidence-before.dump` حفظ شدند.
- طبق تصمیم کاربر هیچ کار بازار، backtest یا timer روزانه‌ای در این مرحله انجام نشد. بدون سند رسمی تازه، ممیزی ۳۳۹ صندوق باقیمانده یا تکرار این promotion مجاز نیست.

## 2026-09-07 - بازیابی شواهدمحور مؤلفه‌های FCFE فولاد

- اسکن read-only متن `1497` فایل candidate و `1234` سند SHA-256-unique نشان داد parser فعلی OCF را می‌خواند اما برچسب‌های واقعی CapEx و تأمین مالی قالب جاری Codal را پوشش نمی‌داد. `343` سند ردیف خرید دارایی ثابت مشهود، `257` سند مؤلفه نامشهود و حدود `236` سند هر دو ردیف دریافت تسهیلات و بازپرداخت اصل داشتند.
- parser اصلاح شد تا `capital_expenditure` فقط با وجود هم‌زمان خرید مشهود و نامشهود در یک جدول جریان نقدی، و `net_borrowing` فقط با وجود هم‌زمان دریافت تسهیلات و بازپرداخت اصل در همان جدول تولید شود. بهره، مانده تسهیلات ترازنامه و تسهیلات اعطایی به دیگران عمداً حذف می‌مانند؛ ردیف غایب صفر فرض نمی‌شود.
- suite کامل data-service برابر `165/165` و CI رسمی PR `#74` سبز شد؛ merge commit `fdcec7067` است. parser در release `20260907T183731Z-main-fdcec7067` نصب شد و سپس drift چند script/test نسبت به main شناسایی گردید.
- از batch ازپیش‌هدف‌گذاری‌شدهٔ فولاد فقط disclosure اصلاحیهٔ سه‌ماهه `1575370` و سالانهٔ تلفیقی حسابرسی‌شده `1574326` انتخاب شدند. دو سند، `24` fact و checksumهای رسمی با `errors=0` اعتبارسنجی شدند. import پس از backup دقیقاً `inserted=4`, `standard_facts=4`, `validation_errors=[]` و replay برابر صفر بود؛ چهار delta جدید همان CapEx و net borrowing دو دوره بودند.
- backup `/var/backups/boursnegar/20260907T183836Z-evidence-before.dump` با `pg_restore -l` و SHA-256 `6d8d95b520f8f128878cb532d6ef73c1fbf590d9ec276210618572a8ff303479` معتبر است. retention دو-dump در dry-run `20260907T184429Z` و apply `20260907T184435Z` اجرا شد؛ dump قدیمی `20260907T124331Z-evidence-before.dump` حذف و مستقیماً قابل‌بازیابی نیست، و dumpهای `182020Z` و `183836Z` حفظ شدند.
- ممیزی نه‌گیتی معتبر Production برای فولاد اکنون OCF، CapEx و net borrowing هم‌دوره را PASS و کل وضعیت را `REVIEW (7/9)` ثبت می‌کند؛ فقط `cost_of_equity` و `terminal_growth` مستند ندارند. ممیزی all-active: `1524` نماد، OCF=`567`، CapEx=`1`، net borrowing=`1`، cost of equity=`0`، terminal growth=`0` و FCFE-ready=`0`. artifact ممیزی SHA-256 `74e5b40423fcef0d3a565a5218bc6c49cada274c3315937128705adc945d3026` است.
- release همگام‌شدهٔ `/var/www/boursnegar-data-releases/20260907T184251Z-main-fdcec7067-sync` فعال است؛ health/readiness و observer موفق، سرویس `active/running` و `NRestarts=0` است. full staging test ابتدا به‌علت فرض مسیر monorepo در پنج test collector متوقف شد؛ runtime switch پیش از تست‌های مرتبط انجام نشد. `44` تست مرتبط روی staging سبز شد و سپس switch انجام گرفت. تست‌ها برای layout مستقل data-service اصلاح شدند تا این preflight دوباره false-fail ندهد.
- پس از PR `#75`، full-suite روی staging تخت `20260907T184848Z-main-35ca47cfc` دوباره اجرا شد: `156` تست پاس و `9` تست monorepo-only به‌علت نیاز به `ops/`، scriptهای ریشه و artifact root بیرون data-service شکست خوردند. staging هرگز فعال نشد، پس از اثبات نبود reference حذف شد و مستقیماً قابل‌بازیابی نیست؛ Production روی release سالم `184251Z` و `/readyz=ready` ماند. پنج collector اصلاح‌شده اکنون مستقل‌اند، اما ادعای full-suite در layout تخت تا جداسازی ۹ contract test مجاز نیست.
- بازار، backtest و timerهای روزانه طبق تصمیم کاربر همچنان خارج از محدوده‌اند و در این مرحله تغییری نکردند.

## 2026-09-07 - اصلاح انتخاب دوره و promotion دقیق FCFE

- batch دوم FCFE ده action معتبر برای پنج سند وارد کرد، اما انتخاب بر پایهٔ آخرین سند کامل محلی بود؛ فقط «دالبر» و «تاتمس» با دورهٔ `rn=1` Production منطبق شدند و شمار CapEx/net borrowing جاری را از `1` به `3` رساندند. سه سند دیگر evidence تاریخی معتبر ماندند ولی گیت جاری را تغییر ندادند.
- برای رفع علت، `2140` کلید دورهٔ جاری Production به‌صورت read-only صادر و candidateها با هویت دقیق `symbol/end_date_jalali/length_months/audited/scope` intersect شدند. نتیجه `96` سند کامل یکتا و `87` candidate منتخب بود؛ پس از حذف actionهای موجود، `84` candidate تازه باقی ماند. artifactهای این ممیزی `production-latest-period-keys-20260907.csv` با SHA-256 `faa3349733fe818c73d3d1859bb9979b798ce2afc3e5f9a712a8b706ae332855` و `fcfe-exact-production-candidates-20260907.json` هستند.
- batch محدود بعدی فقط پنج سند حسابرسی‌شدهٔ دقیقاً منطبق را نگه داشت: «بمپنا» `1590374`، «احیا» `1592987`، «دامین» `1587543`، «وفردا» `1587232` و «آواپارس» `1586797`. پنج سند، `56` fact و manifest با `errors=0` اعتبارسنجی شدند؛ SHA-256 normalized برابر `807dda68a85a2f5e2e465b534c7a1642e2a3db0fb7ce0ed97351a68c18345dd6` است. صفرهای صریح سند پذیرفته شدند و هیچ ردیف غایبی صفر فرض نشد.
- promotion پس از backup `/var/backups/boursnegar/20260907T190638Z-evidence-before.dump` با اندازهٔ `157508281` و SHA-256 `a8891a01b3e51e4b36beafedc98add2b732f92c17df167801f8cef5c6a811665` انجام شد؛ `pg_restore -l` موفق بود. import دقیقاً `inserted=10`, `standard_facts=12`, `validation_errors=[]` و replay برابر `inserted=0`, `standard_facts=0` بود؛ refresh دستی snapshot و readiness موفق شدند.
- ممیزی پس از promotion با SHA-256 `7935f0e765a076d95ea3c5d8e98f942adf2ec11781a6089d317602d6665a21be` نشان داد هر پنج نماد جدید OCF/CapEx/net borrowing هم‌دوره و وضعیت `REVIEW (7/9)` دارند. baseline جاری Production: OCF=`567`، CapEx=`8`، net borrowing=`8`، cost of equity=`0`، terminal growth=`0` و FCFE-ready=`0` از `1524` نماد است.
- retention دو-dump ابتدا در report `20260907T190937Z` dry-run و سپس در `20260907T190953Z` apply شد. dump قدیمی `/var/backups/boursnegar/20260907T183836Z-evidence-before.dump` با SHA-256 `6d8d95b520f8f128878cb532d6ef73c1fbf590d9ec276210618572a8ff303479` حذف و مستقیماً قابل‌بازیابی نیست؛ dumpهای `20260907T185640Z` و `20260907T190638Z` حفظ شدند.
- طبق تصمیم کاربر بازار، backtest و timerهای روزانه همچنان deferred هستند؛ refresh دستی این batch فقط گیت اجباری promotion بود و هیچ timerی تغییر نکرد.

## 2026-09-07 - تکمیل صف اسناد حسابرسی‌شدهٔ FCFE منطبق با دورهٔ جاری

- پس از recheck مستقیم Production، فقط چهار candidate حسابرسی‌شدهٔ تازه و دقیقاً منطبق با دورهٔ جاری باقی مانده بود؛ «انتخاب» به‌دلیل recovery متوقف‌شدهٔ قبلی عمداً کنار گذاشته شد. چهار سند «استقلال» `1525530`، «اعتلا» `1583556`، «امید» `1586523` و «خکرمان» `1581415` شامل `45` fact و دقیقاً `8` action تازهٔ CapEx/net borrowing بودند؛ manifest دارای `errors=0` و SHA-256 normalized برابر `80603940bb731a7be7fd634de863079dc2f80617123651e41d6a67917a2cf71e` است.
- promotion پس از backup `/var/backups/boursnegar/20260907T191447Z-evidence-before.dump` با اندازهٔ `157512335` و SHA-256 `687c7439352b8b60b0efdeaac2dff14d5f1eb7563129e1e77955e2886748e39c` انجام شد؛ `pg_restore -l` موفق بود. import `inserted=8`, `standard_facts=9`, `validation_errors=[]` و replay دقیقاً صفر بود؛ refresh دستی و readiness موفق شدند.
- ممیزی جاری هر چهار نماد را `REVIEW (7/9)` با OCF/CapEx/net borrowing هم‌دوره ثبت کرد. baseline جدید: OCF=`567`، CapEx=`12`، net borrowing=`12`، cost of equity=`0`، terminal growth=`0` و FCFE-ready=`0` از `1524` نماد؛ SHA-256 audit برابر `071c10f92a172cb1073d1d172c6c2922cc49680cb6861e974b2329f55f9d5ed9` است.
- retention در `20260907T191712Z` dry-run و در `20260907T191724Z` apply شد. dump `/var/backups/boursnegar/20260907T185640Z-evidence-before.dump` با SHA-256 `4439a3630f09a1bf1e0150385ff1ba453bf5f93aee8434d0e314aecc0938be9f` حذف و مستقیماً قابل‌بازیابی نیست؛ dumpهای `190638Z` و `191447Z` حفظ شدند.
- feasibility audit نرخ‌ها تأیید کرد `valuation_inputs` فقط چهار NAV معتبر و یک NAV ردشده دارد و هیچ `cost_of_equity`، `wacc` یا `terminal_growth` معتبر ثبت نشده است. جست‌وجوی عنوان disclosure فقط چهار مورد احتمالی یافت: دو ارزش‌گذاری شرکت فرعی برای «درپاد»، یک تغییر برآورد ارزش‌گذاری برای «اروند» و نرخ بازده واحد صندوق «جهش»؛ این عنوان‌ها به‌تنهایی evidence مجاز نرخ issuer-period نیستند و هیچ مقداری promote نشد. artifact متنی SHA-256 `b6684f275c5558990680c58493b5cb1ffe9b90b62a554a340901a37eeaae5e11` است.
- auditor موجود گسترش یافت تا `current_period_keys` را با هویت دقیق symbol/issuer/end/length/audited/scope در همان خروجی ثبت کند؛ این contract از بازگشت انتخاب بر پایهٔ latest محلی جلوگیری می‌کند. suite کامل data-service برابر `166/166` است.
- بازار، backtest و timerهای روزانه همچنان طبق تصمیم کاربر خارج از محدوده‌اند و تغییری نکردند.

## 2026-09-08 - مرکز کنترل گرافیکی کامل Python برای Local تا Production

- ممیزی نشان داد `ingestion_console.py` و `auto_local_to_production.py` از قبل وجود داشتند، اما پاسخ کامل به خواستهٔ کاربر نبودند: default داخلی GUI به DB منسوخ repo-level اشاره می‌کرد، مسیر قدیمی از تب نمادها batch ثابت `1524` را بدون تنظیمات مرکز اجرا می‌کرد، و orchestrator گزارش یکپارچه، اثبات `pg_restore -l` backup، refresh و retention انتهایی نداشت.
- GUI موجود به‌جای ساخت ابزار تکراری به مرکز کنترل واحد ارتقا یافت. تب اصلی اکنون سه سطح صریح دارد: پیش‌بررسی read-only، تکمیل فقط Local، و تکمیل Local تا Production. بازهٔ جلالی و اندازهٔ batch قابل تنظیم، اجرای موازی غیرفعال، همگام‌سازی Production دارای confirmation، و وضعیت Local/Server، progress، تاریخچهٔ runها و log زنده قابل مشاهده‌اند. مسیر canonical DB و run root به‌ترتیب `data-service/artifacts/local-ingestion.sqlite3` و `data-service/artifacts/auto-sync` است.
- backend همین GUI اکنون report ساختاریافته برای dry-run/local/full می‌سازد؛ backup را با size/SHA-256/`pg_restore -l` اعتبارسنجی می‌کند؛ خروجی JSON importer و `validation_errors=[]` را gate می‌کند؛ replay را با `inserted=0` الزام می‌کند؛ سپس refresh دستی snapshot، health/readiness وب و داده، retention dry-run/apply و نتیجهٔ هر مرحله را ثبت می‌کند. بازار یا timer روزانه در این مسیر اجرا یا تغییر داده نمی‌شود.
- اجرای واقعی محدود همین مسیر روی «فجوش» انجام شد. plan برابر `server_symbols=1524`, `selected_symbols=[فجوش]` و بدون failure بود. دو manifest با `17` رکورد تولید شد؛ normalized در اجرای اول `inserted=2`, `standard_facts=3`, `validation_errors=[]` و replay صفر داشت؛ events delta صفر و replay صفر داشت. normalized شامل OCF/CapEx/net borrowing صریح گزارش `1530833` بود و ردیف غایب صفر فرض نشد.
- backup واقعی `/var/backups/boursnegar/20260908T073516Z-auto-local-to-production.dump` با اندازهٔ `157591532`، SHA-256 `21e1b37dd2f0147421f6d334b67d715f9981fb18a781348e99aff58477d19703` و `pg_restore -l=PASS` ثبت شد. refresh دستی و چهار endpoint health/readiness موفق بودند. report محلی در `data-service/artifacts/auto-sync/20260908T073205Z/execution-report.json` و SHA-256 normalized برابر `cda21862c7ed9a5aca597a7e7a88a671a7a0d9cb3d4c0013bb4c387ba38d2c7c` است.
...
## 2026-09-13 - ممیزی بنیادی نمادهای صفحه اصلی و قرنطینه گزارش شرکت فرعی

- فهرست زندهٔ صفحه اصلی از dashboard و ده ردیف نخست screener استخراج شد و ۲۰ نماد یکتا (تیپیکو، شاوان، شاراک، شلرد، حتاید، خفولا، شفا، کشرق، قهکمت، خبازرس، فملی، فولاد، وبملت، شپنا، وتجارت، خودرو، شبندر، وپاسار، خساپا و شتران) تک‌به‌تک refresh و از endpoint عمومی ممیزی شدند. اجرای نهایی `20/20` موفق و selected snapshotها `quarantined=0` و `child_titles=0` بود.
- علت خطای واقعی «وپاسار» و نسبت‌های نامعتبر قبلی، promotion تاریخی گزارش‌هایی بود که عنوانشان با qualifier شرکت/مؤسسهٔ فرعی پایان می‌یافت. importer از این پس raw provenance را حفظ می‌کند ولی چنین رکوردی را به standard fact ارتقا نمی‌دهد. repair غیرحذفی، `6587` fact را از `VALID` به `REVIEW` و `904` snapshot قدیمی را با `evidenceQuarantined=true` علامت زد؛ هیچ سند یا fact فیزیکی حذف نشد.
- مسیرهای عمومی وب، cache تحلیل، API تاریخچه/lineage، coverage audit، calibration و backtest دیگر snapshot قرنطینه‌شده را انتخاب نمی‌کنند. «وپاسار» با آخرین گزارش معتبر غیرشرکت‌فرعی بازسازی شد. خروجی نهایی ۱۶ valuation آماده دارد؛ REVIEWهای شلرد، کشرق، خودرو و خساپا ناشی از زیان/حقوق نامثبت یا ورودی نامعتبر واقعی‌اند و عدد ساختگی صادر نشده است. سه نماد شلرد، وبملت و وپاسار نیز کمبود صریح metric را حفظ می‌کنند.
- backup پیش از تغییر `/var/backups/boursnegar/20260913T093035Z-homepage-analysis-before.dump` با اندازه `159703524`، SHA-256 برابر `d84a43d361b1677b59c7a07e29e8aeb78d6f7e8097d1bbf09cc723e37fb2e139` و `pg_restore -l=PASS` ثبت شد. retention دو-dump اجرا شد. releaseهای فعال data و web به‌ترتیب `20260913T1320Z-snapshot-quarantine` و `20260913T1330Z-snapshot-quarantine` هستند؛ health/readiness و `REMOTE_FINAL_GATE=PASS` موفق‌اند.
- artifact ممیزی نهایی در `data-service/artifacts/audits/homepage-fundamental-20260913/homepage-fundamental-audit-post-quarantine.json` با SHA-256 برابر `ec76f1f08e28b390be8ba92076929942a7f506fb9108a46f9f55f4d52e851c2b` نگه‌داری می‌شود. وب `75/75` تست، typecheck و build را پاس کرد؛ suite کامل data-service در layout صحیح مخزن روی runtime سرور `205 passed, 6 subtests passed` و compile کل app/scripts موفق بود.
- قرارداد coverage نیز industry-aware شد: برای خانواده‌های bank/financial، OCF و EPS دیگر کمبود بنیادی اجباری محسوب نمی‌شوند و پنج ورودی مرتبط revenue/net profit/assets/liabilities/equity معیار پوشش‌اند. پس از deploy release `20260913T1400Z-industry-aware-coverage` و refresh دوبارهٔ هر ۲۰ نماد، ممیزی `20/20` سالم، `quarantined=0`، `child_titles=0` و فقط یک نماد دارای missing metric واقعی شد (قبلاً سه نماد). suite کامل به `206 passed, 6 subtests passed` رسید؛ artifact نهایی `homepage-fundamental-audit-industry-aware.json` با SHA-256 `630ca69c27d3ba0717b9891060a2175d4a86ae2fbbfc12183a4f4bc928c2f6ce` است.
