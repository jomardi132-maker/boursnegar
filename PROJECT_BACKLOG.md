...
- [ ] FCFE/DCF تا وجود همهٔ مؤلفه‌ها و نرخ‌های هم‌دوره و مستند، `INSUFFICIENT_DATA` بماند. baseline فعلی: از ۱۵۲۴ نماد، OCF=`567`، CapEx=`12` و net borrowing=`12`؛ دوازده نماد این سه گیت را هم‌دوره PASS کرده‌اند اما cost of equity و terminal growth کل Production هر دو `0` هستند، پس FCFE-ready همچنان صفر است. candidate بعدی فقط از `current_period_keys` auditor و action تازه انتخاب شود؛ عنوان disclosure یا نرخ بازده صندوق، evidence نرخ issuer-period نیست.
