...
- [x] collector محلی Codal canonical و idempotent است؛ فقط یک‌بار روزانه اجرا می‌شود، partial قدیمی را بدون حذف قرنطینه می‌کند و دیگر توسط coverage دوباره trigger نمی‌شود.
...
- [ ] FCFE/DCF تا وجود همهٔ مؤلفه‌ها و نرخ‌های مستند، `INSUFFICIENT_DATA` بماند. baseline زنده ۱۴۰۵/۰۶/۲۲: OCF=`567`، CapEx=`14`، net borrowing=`14`؛ چهارده نماد سه جزء جریان نقدی و دوازده نماد همه گیت‌های فکت گزارش‌شده را دارند. cost of equity و terminal growth معتبر هر دو `0` و intrinsic-ready صفر است. نرخ‌ها باید policy assumption نسخه‌دار با provenance مستقل باشند؛ عنوان disclosure یا نرخ بازده صندوق evidence نرخ ارزش‌گذاری نیست.
